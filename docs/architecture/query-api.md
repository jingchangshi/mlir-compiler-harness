...
| `attribute-provenance <name>` | attribute_provenance(name) | typed creator provenance (RG-1, ADR-021): td definition join (`attr:<Name>` + DIALECT_OWNS), typed creators (creator_type = OpBuilder / RewritePattern / ConversionPattern / Pass / PipelineBuilder / Verifier, `attach` flag for setAttr lines, per-line evidence), typed consumers (role = verifier / reader), file-level references, and explicit diagnostics (no td definition / no typed creator / definition-only). Ambiguous name → `{"error":"ambiguous","candidates":[...]}`; nothing is guessed |
